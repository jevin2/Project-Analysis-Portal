...
<body>
            <button class="btn"><a href="/demo/student_home.php" style="text-decoration:none;color:white;"><i class="fa fa-home"></i> Home</a></button>
  ...
  $semester = 'sem'.$result['semester']; 
            foreach ($result['semesters'] as $sem=>$sub) {
              ...
              if(isset($rec['rating'])){
                echo "<div class='rating'>RATING: ".$rec['rating']."/5</div>";
            }
              echo "<form action='/demo/editStudent.php' method='post'>
              <input type='hidden' value=".$name." name='sub'>
              <input type='hidden' value=".$semester." name='sem'>
              <input type='hidden' value=".$id." name='id'>
              <input type='text' required class='ratingedit' placeholder='Enter Feedback' name='fdbk'; /><input type='submit' name='feedback' class='sub' value='Submit' /></form></div>";
              #print_r($subclass);
            }
        }?>
